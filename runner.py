...
MAX_TARGET_BYTES = 1 * 1024 * 1024  # 1MB
...
def iterative_render_and_compress(path, w, h):
    ...
    while True:
        pdf_bytes = compose_images_to_target_size(render_pages_to_images(path, dpi), w, h, dpi, q)
        size = len(pdf_bytes)
...
def safe_filename(s):
    ...
    return s + ".pdf" if not s.lower().endswith(".pdf") else s or f"pdf_{int(time.time())}.pdf"
...
def main():
    ...
    ensure_sheet_grid(sheets_svc, SPREADSHEET_ID, SHEET_NAME, 13, 2000)

    cols = sheet_get_columns(sheets_svc, SPREADSHEET_ID, SHEET_NAME, [(COL_URL, START_ROW), (COL_INVOICE, START_ROW)])
    urls, invoices = cols.get(COL_URL, []), cols.get(COL_INVOICE, [])
    for i, url in enumerate(urls, START_ROW):
        if not url:
            continue
        invoice = invoices[i - START_ROW] if i - START_ROW < len(invoices) else ""
        print(f"\nRow {i}: {invoice} → {url}")

        # Skip processed rows
        val = sheets_svc.spreadsheets().values().get(
            spreadsheetId=SPREADSHEET_ID, range=f"{SHEET_NAME}!{_col_letter(COL_OUTPUT)}{i}"
        ).execute().get("values", [])
        ...
        try:
            size = download_url_to_file(drive_svc, url, tmp.name)
            print(f"Downloaded {size} bytes")
            pdf_bytes, final_size, used_dpi, used_quality = iterative_render_and_compress(tmp.name, TARGET_WIDTH_PT, TARGET_HEIGHT_PT)
            fname = safe_filename(invoice or os.path.basename(url.split("?")[0]))
            fid, uploaded_size = upload_file_to_drive_bytes(drive_svc, pdf_bytes, fname, DEST_FOLDER_ID)
            set_file_public_anyone(drive_svc, fid)
            ...
            if BACKUP_ORIGINAL_TO_K:
                try:
                    rng = f"{SHEET_NAME}!K{i}"
                    existing_k = sheets_svc.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=rng).execute().get("values", [])
                    if not existing_k:
                        sheet_update_cell(sheets_svc, SPREADSHEET_ID, SHEET_NAME, i, 11, url)
                except Exception as e:
                    print("Backup K failed:", e)
            sheet_update_cell(sheets_svc, SPREADSHEET_ID, SHEET_NAME, i, COL_OUTPUT, view_url)
            sheet_update_cell(sheets_svc, SPREADSHEET_ID, SHEET_NAME, i, COL_FLAG,
                f"{flag} dpi={used_dpi} q={used_quality} size={uploaded_size}")
            print(f"Row {i}: done → {flag}")
        except Exception as e:
            print("Row error:", e)
            sheet_update_cell(sheets_svc, SPREADSHEET_ID, SHEET_NAME, i, COL_FLAG, f"ERROR: {str(e)[:200]}")
        finally:
            os.remove(tmp.name)
